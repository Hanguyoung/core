    ...
    public sealed class Transmogrifier
    {
                ...
                try
                {
                    ...
                    if (epos < i_len)
                    {
                        ...
                        if ((n_attrs = attrs.Length) != 0)
                        {
                            ...
                            for (i = 0, i_len = n_attrs; i < i_len; i++)
                            {
                                ...
                                if (W3C_2000_XMLNS_URI.Equals(instanceUri) ||
                                    instanceQName.StartsWith("xmlns") && (instanceQName.Length == 5 || instanceQName[5] == ':'))
                                { // i.e. "xmlns" or "xmlns:*"
                                    --n_attrs;
                                    continue;
                                }
                        ...
                        if (useATStarForXsiType || positionOfType != -1)
                        {
                            ...
                            if ((eventTypeForType = matchXsiType(m_scriber.NextEventTypes, useATStarForXsiType)) == null)
                            { // xsi:type had no matching event type
        ...
        private static QName setXsiTypeValue(QName qname, string qualifiedName, PrefixUriBindings namespacePrefixMap)
        {
            ...
            { // with prefix
                ...
                { //  prefix did not resolve into an uri
                    qname.namespaceName = "";
